...
class DocumentGeneratorApp:
    ...
    def setup_ui(self):
        ...
        ttk.Checkbutton(options_frame, text="Generate Markdown files (.md)", 
                       variable=self.generate_md, style='Modern.TCheckbutton').grid(row=1, column=0, sticky=tk.W, pady=5)
    ...
    def start_processing(self):
        ...
        if not self.generate_docx.get() and not self.generate_md.get():
                ...
                try:
                    ...
                    if explanation:
                        ...
                        if self.generate_md.get():
                            md_file = output_path / f"{base_name}_explanation.md"
                            with open(md_file, 'w', encoding='utf-8') as f:
                                f.write(explanation)
                            self.log_result(f"✓ Generated: {md_file.name}")
                        
                        # Generate Word document if requested
                        if self.generate_docx.get():
                            docx_file = output_path / f"{base_name}_explanation.docx"
